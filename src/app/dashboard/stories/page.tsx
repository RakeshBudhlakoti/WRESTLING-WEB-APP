...
export default function MyStories() {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'draft': return 'bg-surface-hover text-muted border-surface-hover';
  ...
  return (
    <div className="flex-1 bg-background text-white pb-20">
      <div className="container mx-auto px-4 mt-12 max-w-5xl">
        ...
        {/* Stories List */}
        <div className="bg-surface border border-surface-hover rounded-3xl overflow-hidden">
          {/* Header */}
          <div className="hidden md:grid grid-cols-12 gap-4 p-6 border-b border-surface-hover text-sm font-bold text-muted uppercase tracking-wider">
              ...
              <div key={story.id} className="p-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-center hover:bg-surface-hover/30 transition-colors">
                ...
                <div className="col-span-2 flex items-center justify-end gap-2">
                  {story.status === 'approved' ? (
                    <Link href={`/stories/${story.id}`} className="w-10 h-10 rounded-full bg-surface hover:bg-surface-hover border border-surface-hover flex items-center justify-center transition-colors text-white" title="View Public">
                      <Eye className="w-4 h-4" />
                    </Link>
                  ) : null}
                  <Link href={`/stories/${story.id}/edit`} className="w-10 h-10 rounded-full bg-surface hover:bg-surface-hover border border-surface-hover flex items-center justify-center transition-colors text-white" title="Edit">
                    <Edit className="w-4 h-4" />
                  </Link>
                  <button className="w-10 h-10 rounded-full bg-surface hover:bg-surface-hover border border-surface-hover flex items-center justify-center transition-colors text-red-500" title="Delete">
